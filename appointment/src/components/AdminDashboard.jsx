    ...
    activeTab === 'overview' && (
        <div className="admin-layout">
            <div className="admin-dashboard-flex">
                ...
                <div className="admin-flex-row">
                    ...
                    <div className="admin-flex-card hero-card">
                        ...
                        <div>
                            ...
                            <div className="hero-trend">With recent activity</div>
                        </div>
                    </div>
                    ...
                    <div className="admin-flex-card hero-card">
                        ...
                        <div>
                            <div className="hero-label">System Health</div>
                            <div className="hero-value" style={{ fontSize: '1.25rem' }}>Operational</div>
                            <div className="hero-trend">All systems normal</div>
                        </div>
                    </div>
                </div>
                ...
                <div className="admin-flex-row">
                    {/* Trend Chart */}
                    <div className="admin-flex-card chart-card-wide">
                        <div className="admin-card-header">
                            <h3 className="admin-card-title">Appointment Trends</h3>
                            <p className="admin-card-subtitle">Global booking volume (Last 6 Months)</p>
                        </div>
                            ...
                            <ResponsiveContainer>
                                <LineChart data={appointmentTrend}>
                                    ...
                                    <Line
                                        ...
                                        stroke="#2452a0"
                                        strokeWidth={3}
                                        dot={{ r: 4, fill: '#2452a0', strokeWidth: 2, stroke: '#fff' }}
                                        activeDot={{ r: 6 }}
                                    />
                                </LineChart>
                            </ResponsiveContainer>
                    ...
                    {/* Status Pie Chart */}
                    <div className="admin-flex-card chart-card-narrow">
                        <div className="admin-card-header">
                            <h3 className="admin-card-title">Status Overview</h3>
                            <p className="admin-card-subtitle">Current distribution</p>
                        </div>
                        <div style={{ width: '100%', height: 300 }}>
                            {(() => {
                                let confirmed = 0, completed = 0, cancelled = 0;
                                ...
                                const data = [
                                    { name: 'Confirmed', value: confirmed, color: '#2452a0' },
                ...
                {/* 3. Leaderboard Table */}
                <div className="admin-flex-card">
                    <div className="admin-card-header">
                        <h3 className="admin-card-title">Top Performing Clinics</h3>
                        <p className="admin-card-subtitle">Ranked by appointment volume</p>
                    </div>
                        ...
                        <table className="admin-table">
                            ...
                            <tbody>
                                {clinicSummaries
                                    ...
                                    .slice(0, 5)
                                    .map((clinic, i) => {
                                        // Calculate success rate (completed / total) roughly
                                        const details = clinicDetails[clinic.id] || {};
                                        const appts = details.appointments || [];
                                        const completed = appts.filter(a => a.status === 'completed').length;
                                        const rate = appts.length > 0 ? Math.round((completed / appts.length) * 100) : 0;

                                        return (
                                            <tr key={clinic.id}>
                                                <td>
                                                    <span className="leaderboard-rank-badge">#{i + 1}</span>
                                                </td>
                                                <td className="font-medium">{clinic.name}</td>
                                                <td className="text-secondary">{clinic.city}</td>
                                                <td>
                                                    <span className={`admin-tag ${clinic.status === 'active' ? 'success' : 'warning'}`}>
                                                        {clinic.status}
                                                    </span>
                                                </td>
                                ...
                                {clinicSummaries.length === 0 && (
                                    <tr><td colSpan="6" className="text-center py-4 text-muted">No active clinics found</td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
